...
let check_cubes cube1 cube2= 
cube1 = cube2 


let turn_tests (name : string) input1 input2 : test =
  name >:: fun _ -> assert_equal true (check_cubes input1 input2)

let u'_red_face = [| Red; Red; Red; Red; Red; Red; Blue; Blue; Blue |]
let dupe_u'_red_face = [| Red; Red; Red; Red; Red; Red; Blue; Blue; Blue |]
...
let dupe_u'_green_face = [| Green; Green; Green; Green; Green; Green; Red; Red; Red |]
...
let dupe_u'_orange_face = [| Orange; Orange; Orange; Orange; Orange; Orange; Green; Green; Green |]
...
let dupe_u'_blue_face = [| Blue; Blue; Blue; Blue; Blue; Blue; Orange; Orange; Orange |]
...
let dupe_u'_cube = [| dupe_u'_red_face; yellow_face; dupe_u'_blue_face; white_face; dupe_u'_orange_face; dupe_u'_green_face |] 
...
let () = cube |> u'_turn
let () = dupe_u'_cube |> u'_turn
let u'_tests =
  [
    turn_tests "default cube; testing one u' turn on the default cube" u'_cube cube;
    turn_tests
      "testing two u' turns on the default cube (i.e., one u' turn from\n\
      \  the cube in the state directly after one u' turn)" u'_u'_cube dupe_u'_cube;
    (* turn_tests
